...
// Agency Career boots as a closed digital-agency loop by default and exposes auditable management mechanics.
{
  ...
  assert.equal(s.agencyModelVersion,CURRENT_AGENCY_MODEL);assert.equal(value(context,"AgencyCareer.modelVersion"),CURRENT_AGENCY_MODEL);
...
}
{
  ...
  assert.equal(value(context,"AgencyCareer.clientServicesOf(S.clients[0]).length"),0);
  assert.equal(value(context,"AgencyCareer.canSellService(S.clients[0],'email_program',S).ok"),false,"a service sold without its capability");
  vm.runInContext("S.unlocked.push('measurement','landing_systems');S.clients[0].trust=70;S.cash=500000;S.focusTotal=20;S.focusRemaining=20",context);
  const fee=value(context,"S.clients[0].fee");
  assert.equal(value(context,"AgencyCareer.sellClientService(S.clients[0].id,'email_program',{render:false})"),true);
  assert(state(context).clients[0].fee>fee,"selling a service did not raise the retainer");
  assert.equal(value(context,"AgencyCareer.canSellService(S.clients[0],'email_program',S).ok"),false,"the same service sold twice");
  const strength=state(context).clients[0].services.email_program;
  assert.equal(value(context,"AgencyCareer.workClientService(S.clients[0].id,'email_program',{render:false})"),true);
  assert(state(context).clients[0].services.email_program>strength,"working the programme did not strengthen it");
  // Conversion work lifts outcomes on the same media spend.
  const plain=makeContext("?mode=6&budget=250000&seed=1672&agencyType=digital_agency&hq=portland-or");
  vm.runInContext("S.clients[0].nextDue=999;AgencyCareer.runDay({force:true})",plain.context);
  vm.runInContext("S.clients[0].nextDue=999;S.clients[0].services={email_program:100};AgencyCareer.runDay({force:true})",context);
  const withService=state(context).clients[0].campaignHistory.at(-1),without=state(plain.context).clients[0].campaignHistory.at(-1);
  assert.equal(withService.spend,without.spend,"a sold service changed the client's media spend");
  assert(withService.leads>without.leads,"conversion work bought no extra outcomes on the same spend");
  // And it fades when nobody works it.
  const before=state(context).clients[0].services.email_program;
  vm.runInContext("AgencyCareer.runDay({force:true})",context);
  assert(state(context).clients[0].services.email_program<before,"a neglected programme did not fade");
  assert.equal(value(context,"AgencyCareer.validate(S)"),true,"client services broke save validation");
}
{
...
{
  /* The levers modes 1-4 always had, now in agency: kill a loser, pick the landing page.
     Pausing must move budget onto what is still live rather than shrinking the account, and
     the landing page must move conversions without moving what a click costs. */
  const {context}=makeContext("?mode=6&budget=25000&seed=1669");
  const clientId=value(context,"S.clients[0].id");
  vm.runInContext("S.focusRemaining=8;S.cash=400000;S.unlocked.push(\"campaign_structure\")",context);
  vm.runInContext(`AgencyCareer.splitAccount(${JSON.stringify(clientId)},{render:false})`,context);
  assert.equal(state(context).clients[0].campaigns.length,2,"the account did not split");
  const baseline=JSON.parse(value(context,"JSON.stringify(AgencyCareer.export())"));

  // Pausing the only live campaign is refused, and refusing leaves the save byte-identical.
  vm.runInContext("S.focusRemaining=8;S.clients[0].campaigns[1].paused=true",context);
  const guarded=JSON.parse(value(context,"JSON.stringify(AgencyCareer.export())"));
  vm.runInContext(`AgencyCareer.toggleCampaignPaused(${JSON.stringify(clientId)},S.clients[0].campaigns[0].id,{render:false})`,context);
  assert.equal(value(context,"JSON.stringify(AgencyCareer.export())"),JSON.stringify(guarded),
    "pausing the last live campaign was allowed, or the refusal mutated state");

  /* A single day yields under one conversion and the report rounds to a tenth, so a real
     lift hides inside the rounding. Sum a fortnight instead. */
  const runDay=(days=12)=>{let spend=0,leads=0;
    for(let i=0;i<days;i++){vm.runInContext("S.focusRemaining=8;AgencyCareer.runDay()",context);
      const report=state(context).lastDayReport;spend+=report.spend;leads+=report.leads;}
    return {spend:Math.round(spend),leads:Math.round(leads*10)/10};};
  /* Two freshly split campaigns are identical, so pausing one and handing its budget to its
     twin is genuinely a no-op. Make them differ first: campaign 0 gets the better landing
     page, so killing campaign 1 must concentrate spend on the stronger of the two. */
  vm.runInContext(`AgencyCareer.hydrate(${JSON.stringify(baseline)})`,context);
  vm.runInContext(`S.focusRemaining=8;S.cash=400000;AgencyCareer.setCampaignLander(${JSON.stringify(clientId)},S.clients[0].campaigns[0].id,"simple_form",{render:false})`,context);
  const mixed=JSON.parse(value(context,"JSON.stringify(AgencyCareer.export())"));
  const live=runDay();
  vm.runInContext(`AgencyCareer.hydrate(${JSON.stringify(mixed)})`,context);
  vm.runInContext(`S.focusRemaining=8;AgencyCareer.toggleCampaignPaused(${JSON.stringify(clientId)},S.clients[0].campaigns[1].id,{render:false})`,context);
  assert.equal(state(context).clients[0].campaigns[1].paused,true,"the campaign did not pause");
  const killed=runDay();
  assert(Math.abs(killed.spend-live.spend)<=live.spend*0.02,
    `a paused campaign changed total spend (${live.spend} -> ${killed.spend}); its share must move to what is still live`);
  assert(killed.leads>live.leads,
    `killing the weaker campaign did not concentrate spend on the stronger one (${live.leads} -> ${killed.leads})`);

  vm.runInContext(`AgencyCareer.hydrate(${JSON.stringify(baseline)})`,context);
  vm.runInContext("S.focusRemaining=8;S.cash=200000",context);
  const before=runDay();
  vm.runInContext(`AgencyCareer.hydrate(${JSON.stringify(baseline)})`,context);
  vm.runInContext(`S.focusRemaining=8;S.cash=200000;AgencyCareer.setCampaignLander(${JSON.stringify(clientId)},S.clients[0].campaigns[0].id,"simple_form",{render:false})`,context);
  assert.equal(state(context).clients[0].campaigns[0].lander,"simple_form","the landing page did not change");
  const after=runDay();
  assert(Math.abs(after.spend-before.spend)<=before.spend*0.02,
    `the landing page moved media spend (${before.spend} -> ${after.spend}); it must only move conversion rate`);
...
}
{
  ...
  assert.equal(state(context).agencyModelVersion,CURRENT_AGENCY_MODEL);
  assert.equal(state(context).clients[0].strategy,"balanced");
  assert.equal(value(context,"AgencyCareer.validate(S)"),true);
}
{
  ...
  assert.equal(repaired.agencyModelVersion,CURRENT_AGENCY_MODEL);
...
// A real legacy-v1 browser checkpoint migrates through session restore without losing or doubling accrued variable costs.
{
  ...
  assert.equal(s.agencyModelVersion,CURRENT_AGENCY_MODEL);assert.equal(s.monthVariableCosts,1750);assert.equal(s.monthCostLedger.other,1750);
...
// adds origin, geography, offer and ad fields without rerolling established money or client facts.
{
  ...
  assert.equal(state(source.context).agencyModelVersion,CURRENT_AGENCY_MODEL);assert.equal(value(source.context,"AgencyCareer.validate(S)"),true,
  ...
  assert.equal(s.agencyModelVersion,CURRENT_AGENCY_MODEL);assert.deepEqual({...s.agencyIdentity},{name:"Moonrise Media",hqId:"portland-or",agencyType:"digital_agency"});
...
// with another offer's concept. Migration repairs the creative without rerolling money or client history.
{
  ...
  assert.equal(repaired.agencyModelVersion,CURRENT_AGENCY_MODEL);assert.equal(repaired.cash,cash);assert.equal(client.trust,trust);
