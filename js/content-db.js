...
const PROFILE_DB=Object.freeze({
  general:Object.freeze({
    ...
    scope:"Cross-platform media buying · paid search, paid social, display, CTV, client, in-house, and agency-career practice",
...
const CREATIVE_FORMATS=Object.freeze({
  ...
  vsl:Object.freeze({id:"vsl",label:"VSL",mark:"🎬",tone:"violet",system:"authority",kind:"persuasion structure",
  ...
  veo:Object.freeze({id:"veo",label:"VEO (AI-gen video)",mark:"✨",tone:"pink",system:"hook",kind:"production method",
...
const LEGACY_CREATIVE_FORMATS=Object.freeze({
  ...
  ugc:Object.freeze({id:"ugc",label:"UGC video · legacy save",mark:"🤳",tone:"pink",system:"hook",kind:"legacy format taxonomy",
  ...
  ctv:Object.freeze({id:"ctv",label:"CTV spot · legacy save",mark:"📺",tone:"amber",system:"authority",kind:"legacy format taxonomy",
...
const GUIDED_PLAYBOOK=Object.freeze([
  ...
  {id:"02",title:"Variation axes",summary:"Multiply a proven concept along controlled axes before replacing it with unrelated ideas.",
    ...
    advanced:"An axis is useful only when the resulting variants remain attributable, compliant, and sufficiently powered to compare. Equal ad rotation improves readability but does not guarantee a conclusive experiment, and the trainer's rapid-scale threshold is an operational game rule rather than a platform benchmark.",
  ...
  {id:"04",title:"Winner and anomaly lab",summary:"The most instructive rows are often the ones whose metrics disagree.",
    ...
    operator:"Trace each declared path from its denominator to its outcome before assigning a cause. In Modes 1–4, click-to-lead CVR and the landing-page diagnostic are parallel branches after the ad click.",
  ...
  {id:"05",title:"Hierarchy, navigation, and diagnostic order",summary:"Know which object is being observed or changed: account, campaign, group, ad, creative, keyword diagnostic, or measurement source.",
    core:"An ad carries creative. A campaign declares structure and objective. An account holds permissions, billing, and delivery history. In paid search, Quality Score is a keyword-level diagnostic assembled from expected CTR, ad relevance, and landing page experience.",
    ...
    advanced:"Quality Score is neither a KPI nor the literal auction-time input. A dedicated campaign and campaign delivery pacing change structural control, not ad quality. Reporting keys, event-source mappings, optimization sources, and diagnostic scores may overlap operationally, but they are not interchangeable. Brand-keyword conquest can also capture demand another channel created.",
  ...
  {id:"08",title:"Questions for the account lead",summary:"Turn unknown account context and client preferences into explicit, testable questions instead of unsupported conclusions.",
    ...
    advanced:"Questions should discriminate among account and relationship hypotheses. Track results confidence, judgment, transparency, responsiveness, and alignment separately from short-term tension. Evidence and operational quality outrank stylistic fit; later behavior must verify each commitment. The client-retention line is a simulation boundary, not a universal personality score. A client seat measures agency capacity, while platform ad accounts remain delivery objects inside that relationship.",
    checklist:["Prioritize three decision-changing questions.","Confirm the source, pace window, and goal.","Label the business prior as uncertain.","Check the service cadence and any service debt.","Compare focus demand with team capacity before promising more.","Record the reaction and update the Client Read.","Close or renegotiate every commitment."],terms:["intake","baseline","monthly conversion pace","campaign source of truth","business prior","client trust","trust dimensions","client tension","communication stance","client insight","client read","working agreement","commitment","client retention line","relationship risk","client seat","service cadence","service debt","focus units","capacity utilization","sprawl penalty"]},
  ...
  {id:"11",title:"Compliance, claims, and asset rights",summary:"Creative sourcing includes rights, brands, people, claims, and jurisdiction-specific review.",
    ...
    checklist:["Verify usage rights.","Scan every frame for marks and people.","Qualify claims.","Escalate uncertainty before shipping."],terms:["compliance","compliance health","approval","account hold","creative pipeline"]},
...
const TUTORIAL_DB=Object.freeze({
  ...
  actions:Object.freeze([
    Object.freeze({id:"baseline",kind:"run",focus:"runBtn",title:"Establish the untouched baseline",instruction:"Select RUN DAY without changing anything.",body:"A baseline gives every later change something real to compare against. One period is evidence, not a trend."}),
    Object.freeze({id:"lens",kind:"view",focus:"viewBtn",title:"Compare the two reporting lenses",instruction:"Switch the account lens from modeled outcome to attributed report.",body:"The business outcome and the platform-attributed report can be internally consistent while answering different questions."}),
    Object.freeze({id:"ask",kind:"slot",action:"ask",target:"brand",focus:"slots",title:"Ask before judging intent",instruction:"Use Ask the buyer on the reach-objective slot.",body:"A negative in-window line item may be intentional. Reveal the objective before deciding that the ad failed."}),
    Object.freeze({id:"multiply",kind:"slot",action:"mult",target:"utility",focus:"slots",title:"Multiply a proven concept",instruction:"Multiply the Bill Screenshot workhorse on one declared axis.",body:"A controlled variation refreshes a proven idea. It is not the same thing as inventing a new concept."}),
    Object.freeze({id:"request",kind:"creative_request",format:"static",focus:"pipeBox",title:"Create one test option",instruction:"Open the Creative Lab and test Static.",body:"Format is the execution, concept is the idea, and rarity is the simulation's upside roll. Fundamentals delivers tests instantly; the tutorial fixes the format choice, and the seeded rarity remains reproducible."}),
    Object.freeze({id:"swap",kind:"creative_swap",target:"trap",focus:"slots",title:"Swap creative into an existing ad slot",instruction:"Ship the new creative into the weak Mobile Broad slot.",body:"The ad slot, account, and campaign remain. Only the creative carried by that ad changes."}),
    Object.freeze({id:"comparison",kind:"run",focus:"runBtn",title:"Run the comparison period",instruction:"Select RUN DAY again.",body:"Now the board can compare a deliberate intervention with the untouched baseline. One comparison is evidence, not yet a trend."}),
    Object.freeze({id:"allocate",kind:"slot",action:"plus",target:"best",focus:"slots",title:"Allocate toward the strongest current signal",instruction:"Increase the highlighted slot by one budget step.",body:"Allocation is the portion of available buying authority assigned to a delivery slot. This small step tests scale without confusing a giant budget jump with creative quality."}),
    Object.freeze({id:"window",kind:"run",focus:"runBtn",title:"Extend the evidence window",instruction:"Select RUN DAY for the third guided period.",body:"A third period begins an evidence window: the player can now distinguish a repeatable signal from one lucky observation. After it resolves, the full account opens."})
  ])
});
