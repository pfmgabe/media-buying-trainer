...
const CREATIVE_FORMATS=Object.freeze({
  static:Object.freeze({id:"static",label:"Static image",mark:"🖼️",tone:"cyan",
  ...
  motion:Object.freeze({id:"motion",label:"Motion graphic",mark:"🎞️",tone:"amber",
  ...
  founder:Object.freeze({id:"founder",label:"Founder / explainer",mark:"🗣️",tone:"violet",
  ...
  utility:Object.freeze({id:"utility",label:"Input / UI utility",mark:"🖥️",tone:"blue",
  ...
  search:Object.freeze({id:"search",label:"Search text / assets",mark:"🔍",tone:"blue",
...
const GUIDED_PLAYBOOK=Object.freeze([
  ...
  {id:"03",title:"Unit economics and calibration",summary:"Read cost and downstream value together; engagement alone cannot establish business quality.",
    core:"CPL describes acquisition cost. EPL describes value per lead. Profit depends on both. Attributed media margin is a currency amount—attributed value minus media spend—while ROI is a percentage based on a declared cost base.",
    operator:"Use account-specific normal ranges as investigation context, never universal truth. Align numerator, denominator, attribution, and date window. Compare marginal MER with the blended result before committing the next allocation step.",
    advanced:"Cohort quality, settlement lag, and attribution uncertainty matter more than a single blended average at scale. Working capital can fail before profitable receivables are collected; taking an early receivable haircut exchanges contribution for liquidity rather than repairing media performance.",
    checklist:["Pair CPL with EPL.","Check modeled and attributed value.","Distinguish margin amount from ROI percentage.","Align windows.","Inspect marginal performance before scaling.","Reconcile collections and due bills before declaring cash safe."],terms:["cpl","epl","profit","attributed media margin","attribution gap","marginal mer","working capital","receivable collections","receivable haircut","credit payment failure"]},
  ...
  {id:"08",title:"Questions for the account lead",summary:"Turn unknown account context and client preferences into explicit, testable questions instead of unsupported conclusions.",
    core:"Ask what the campaign is for, which baseline and campaign source of truth govern it, what constraints apply, and who owns the next decision. Treat the business context as a starting hypothesis, never a personality verdict. Monthly conversion pace is a projection—not a promise of the final total.",
    operator:"During pressure, separate account facts from an observable client cue. Choose a communication stance—recommend, investigate, acknowledge, or set a boundary—then record what the reaction taught you. Convert the response into a working agreement with an owner, evidence check, and due point.",
    advanced:"Questions should discriminate among account and relationship hypotheses. Track results confidence, judgment, transparency, responsiveness, and alignment separately from short-term tension. Evidence and operational quality outrank stylistic fit; later behavior must verify each commitment. The client-retention line is a simulation boundary, not a universal personality score.",
    checklist:["Prioritize three decision-changing questions.","Confirm the source, pace window, and goal.","Label the business prior as uncertain.","Separate facts from observable cues.","Record the reaction and update the Client Read.","Name the owner, evidence check, and due point.","Close or renegotiate every commitment."],terms:["intake","baseline","monthly conversion pace","campaign source of truth","business prior","client trust","trust dimensions","client tension","communication stance","client insight","client read","working agreement","commitment","client retention line","relationship risk"]},
...
const TUTORIAL_DB=Object.freeze({
  ...
  reveal:Object.freeze([
    ...
    Object.freeze({target:"slot-0",title:"3 · Follow one card through its evidence paths",body:"Read impressions → clicks → modeled leads for the outcome path. Then inspect landing visits → on-page actions as a parallel LP-CTR diagnostic; it is not multiplied into CVR."}),
  ...
  coach:Object.freeze([
    Object.freeze({throughDay:1,title:"Baseline day",body:"Run Day 1 with the starting allocation. Watch account economics, the click-to-lead outcome path, and the separate landing diagnostic resolve before changing a control.",focus:"runBtn"}),
